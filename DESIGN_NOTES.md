...
## 4. customerDocId と authUid は意図的に分離（v8.1 改訂・重要度：最高）

> ★ 2026/5/17 全面改訂。旧 v6 は「customerId = Auth UID に統一、
> 分離は YAGNI で先送り」だったが、サロン側顧客登録（Auth 未登録の
> 電話予約客）を入れる要件で前提が崩れ、v7→v8→v8.1 の3者合意で
> **意図的分離**に転換した。詳細は `DESIGN_v8_1_customer_identity.md`。

### 確定方針（v8.1）
- 顧客カルテ ID は **`customerDocId`（Firestore 採番）**
- 顧客の Auth UID は **`authUid`**（アプリ登録時のみ存在、サロン
  仮登録は null）
- 両者は別概念。`customerId` という曖昧名は新コードで**使用禁止**
- `appointments` も `customerDocId` に改名＋`authUid`＋
  `customerSnapshot{name,phone}`（予約時点・不変）を持つ

### source of truth
- **`authIndex/{authUid}` が source of truth**（authUid→customerDocId）
- `customers.authUid` はキャッシュ（写し）
- 両者の更新は Cloud Function のみ。不一致時は authIndex を正とし
  customers.authUid を直す。整合監査 Function を将来用意

### claim 条件 B（emailVerified 必須メール一致）
- アプリ登録で emailVerified==true かつ email 完全一致の未 claim
  カルテがちょうど1件 → そのカルテに authUid 付与（既存カルテに claim）
- 不成立は新カルテ＋サロンに統合候補提示（人間判断）
- 電話番号 claim は本人証明できず乗っ取りリスクのため不採用

---

## 4-b. 顧客統合（merge）の鉄則（v8.1・重要度：最高）

- **callable Function のみ。クライアント batch 禁止**
  （旧 TORITA は client batch かつ「失敗を成功と表示」する
  致命的欠陥があった。これは**再発禁止**）
- 統合元は**物理削除しない soft delete**
  （`isMerged/mergedInto/mergedAt`、`mergedAliases` に旧ID）。
  理由：誤統合 rollback・監査・税務・履歴
- `stampCount`/`totalSpent` は**履歴から再計算が正**。
  単純合算は二重加算事故になるため暫定扱いと明記する
- 予約付替は current + archive 両方。customerSnapshot は不変
- **排他制御**：merge 開始時に Firestore トランザクションで
  src/dst を `lockedByJob` ロック。`mergeJobs` でジョブ状態管理
  （pending/processing/completed/failed）。順序は
  予約付替→完了確認→authIndex更新→src soft delete。
  完了まで src を soft delete しない（failed 時リトライ可能）。
  15分超 processing の stale ジョブ安全解放 Function を将来用意
- merge 時の authUid 規則：1 authUid = 1 customerDocId。
  src/dst 両方 authUid あり（別値）は自動 merge 禁止＝人間が選択
